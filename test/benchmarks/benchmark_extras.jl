...
function benchmark_mi_single()
    ...
    time_repeat(
        "Small (100)",
        () -> mi_continuous_discrete(x_small, y_small; n_neighbors=3, rng=rng),
    )
    ...
    time_repeat(
        "Medium (1000)",
        () -> mi_continuous_discrete(x_medium, y_medium; n_neighbors=3, rng=rng),
    )
    ...
    time_repeat(
        "Large (5000)",
        () -> mi_continuous_discrete(x_large, y_large; n_neighbors=3, rng=rng),
        3,
    )
...
function profile_allocations()
    ...
    println(
        "  compute_mi_cd: $(round(result.time * 1000, digits=2)) ms, $(round(result.bytes / 1024^2, digits=2)) MB allocated",
    )

    # Profile mi_continuous_discrete (single)
    result = @timed mi_continuous_discrete(x_single, y; n_neighbors=3, rng=rng)
    println(
        "  mi_continuous_discrete (single): $(round(result.time * 1000, digits=2)) ms, $(round(result.bytes / 1024^2, digits=2)) MB allocated",
    )

    # Profile mi_continuous_discrete (multiple)
    result = @timed mi_continuous_discrete(X, y; n_neighbors=3, rng=rng)
    println(
        "  mi_continuous_discrete (multiple): $(round(result.time * 1000, digits=2)) ms, $(round(result.bytes / 1024^2, digits=2)) MB allocated",
    )
...
function run_benchmarks()
    ...
    return println("✅ Benchmark complete! Check results above for performance bottlenecks.")
end
